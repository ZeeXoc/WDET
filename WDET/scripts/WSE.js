if (!WDET.pasteMod)  {
    WDET.pasteMod = {
        now: "off", q: function (v) {
            ...
            var t = this.q("#edit-page-textarea");
            var all = t.value;
            ...
            var ev = this.q("#edit-page-textarea").value;
            ...
            try {
                var meta = this.q('[name="viewport"]');
                meta.setAttribute("content", meta.getAttribute("content") + ",user-scalable=no");
            } catch (e) {
            }
            if (!this.q("#edit-page-textarea")) {
                if (!window.WIKIDOT) {
                    return alert("Detection failed(检测失败): WIKIDOT object is undefined");
                } else {
                    try {
                        WIKIDOT.page.listeners.editClick();
                        var target = this.q("#action-area");
            ...
            var t = this.q("#edit-page-textarea");
            var e = this.q("#action-area button[data-j]");
